...
const ChatScreen = ({ setIsAuthenticated, firstName, theme, toggleTheme }) => {
  ...
  const [cameraStream, setCameraStream] = useState(null);
  const [enlargedImage, setEnlargedImage] = useState(null);
  const [isVideoReady, setIsVideoReady] = useState(false); // New state to track video readiness
  const chatContentRef = useRef(null);
  const fileInputRef = useRef(null);
  const videoRef = useRef(null);
  const [copiedStates, setCopiedStates] = useState({});
  ...
  useEffect(() => {
    ...
    setIsMobile(/android|iPhone|iPad|iPod|webOS|BlackBerry|Windows Phone/i.test(userAgent));
    setShowWelcome(true);
    setShowTitle(true);
  }, [firstName]);
    ...
    if (videoRef.current && cameraStream) {
      ...
      video.onloadedmetadata = () => {
        video.play().then(() => {
          ...
          console.log('Video stream is ready:', video.videoWidth, 'x', video.videoHeight);
        }).catch((err) => {
          console.error('Error playing video:', err);
          alert('Failed to play video stream. Please try again.');
        });
      };
      video.onerror = (err) => {
  ...
  const handleOpenCamera = async () => {
    try {
      ...
      console.error("Error accessing camera:", err);
      alert("Could not access the camera. Please check permissions or try uploading an image from your gallery.");
    }
  };

  const handleCapturePhoto = () => {
    if (!videoRef.current || !isVideoReady) {
      alert('Camera is not ready yet. Please wait a moment or try reopening the camera.');
      return;
    }
    ...
    if (videoWidth === 0 || videoHeight === 0) {
      alert('Unable to capture photo: Video dimensions are not available. Please try reopening the camera.');
      console.error('Video dimensions are zero:', videoWidth, 'x', videoHeight);
      return;
    }
    ...
    canvas.toBlob((blob) => {
      ...
      setSelectedImage(file);
      stopCamera();
    }, "image/jpeg", 0.9); // Added quality parameter for better compression
  };
            ...
            {cameraStream && (
              <div className="camera-modal">
                <video
                  ref={videoRef}
                  autoPlay
                  playsInline // Added playsInline for better compatibility
                  className="camera-video"
                />
                <button
                  onClick={handleCapturePhoto}
